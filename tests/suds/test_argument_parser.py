...
import virtwho.virt.esx.suds.argparser
...
def test_binding_uses_argument_parsing(monkeypatch, binding_style):
    ...
    def raise_exception(*args, **kwargs):
        raise MyException
    monkeypatch.setattr(virtwho.virt.esx.suds.argparser._ArgParser, "__init__", raise_exception)

    wsdl = virtwho.virt.esx.suds.byte_str("""\
...
def test_extra_positional_arguments(param_optional, args):
    ...
    _expect_error(TypeError, expected, virtwho.virt.esx.suds.argparser.parse_args, "fru-fru",
...
def test_multiple_value_for_single_parameter_error(param_names, args, kwargs):
    ...
    if len(expected) == 1:
        expected = expected[0]
    _expect_error(TypeError, expected, virtwho.virt.esx.suds.argparser.parse_args, "q", params,
...
def test_not_reporting_extra_argument_errors():
    ...
    param_processor = MockParamProcessor()
    args_required, args_allowed = virtwho.virt.esx.suds.argparser.parse_args("w", params, args,
...
def test_unexpected_keyword_argument(param_names, args, kwargs):
    ...
    if len(expected) == 1:
        expected = expected[0]
    _expect_error(TypeError, expected, virtwho.virt.esx.suds.argparser.parse_args, "pUFf",
