...
"""
Suds Python library virtwho.virt.esx.suds.client.Client related unit tests.
...
import virtwho.virt.esx.suds.cache
import virtwho.virt.esx.suds.store
import virtwho.virt.esx.suds.transport
...
class MockCache(virtwho.virt.esx.suds.cache.Cache):
    """
...
class MockDocumentStore(virtwho.virt.esx.suds.store.DocumentStore):
...
class MockTransport(virtwho.virt.esx.suds.transport.Transport):
    """
    ...
    def open(self, request):
        ...
        return virtwho.virt.esx.suds.BytesIO(result)
...
class TestCacheStoreTransportUsage:
    """
    virtwho.virt.esx.suds.client.Client cache/store/transport component usage interaction tests.
    ...
    class TestCachedWSDLObjectUsage:
        ...
        def test_avoid_imported_WSDL_fetching(self):
            ...
            c1 = virtwho.virt.esx.suds.client.Client("suds://wsdl", cachingpolicy=1,
            ...
            c2 = virtwho.virt.esx.suds.client.Client("suds://wsdl", cachingpolicy=1,
...
<schema xmlns="http://www.w3.org/2001/XMLSchema">
    ...
    def test_importing_WSDL_from_cache_avoids_store_avoids_transport(self,
        ...
        store1 = MockDocumentStore(wsdl=wsdl_import_wrapper,
            wsdl_imported=wsdl_imported)
        c1 = virtwho.virt.esx.suds.client.Client("suds://wsdl", cachingpolicy=0, cache=cache,
        ...
        if importing_WSDL_cached:
            ...
            assert len(cache.mock_data) == 1
            store2 = MockDocumentStore(wsdl=wsdl_import_wrapper)
        c2 = virtwho.virt.esx.suds.client.Client("suds://wsdl", cachingpolicy=0, cache=cache,
    ...
    def test_using_cached_WSDL_avoids_store_avoids_transport(self,
        ...
        c1 = virtwho.virt.esx.suds.client.Client("suds://umpala", cachingpolicy=caching_policy,
            cache=cache, documentStore=store1, transport=MockTransport())
        ...
        cache.mock_put_config = MockCache.FAIL
        store2 = MockDocumentStore(mock_fail=True)
        c2 = virtwho.virt.esx.suds.client.Client("suds://umpala", cachingpolicy=caching_policy,
            cache=cache, documentStore=store2, transport=MockTransport())
...
class TestCacheUsage:
    """virtwho.virt.esx.suds.client.Client cache component usage tests."""

    @pytest.mark.parametrize("cache", (
        None,
        virtwho.virt.esx.suds.cache.NoCache(),
        virtwho.virt.esx.suds.cache.ObjectCache()))
    def test_avoiding_default_cache_construction(self, cache, monkeypatch):
        ...
        class MockStore(virtwho.virt.esx.suds.store.DocumentStore):
            def open(self, *args, **kwargs):
                raise MyException
        monkeypatch.setattr(virtwho.virt.esx.suds.cache, "ObjectCache", construct_default_cache)
        monkeypatch.setattr(virtwho.virt.esx.suds.store, "DocumentStore", MockStore)
        pytest.raises(MyException, virtwho.virt.esx.suds.client.Client, "suds://some_URL",
    ...
    def test_reject_invalid_cache_class(self, cache, monkeypatch):
        ...
        e = pytest.raises(AttributeError, virtwho.virt.esx.suds.client.Client,
            "suds://some_URL", cache=cache).value
        try:
            expected_error = '"cache" must be: (%r,)'
            assert str(e) == expected_error % (virtwho.virt.esx.suds.cache.Cache,)
        finally:
            del e  # explicitly break circular reference chain in Python 3


class TestStoreUsage:
    """virtwho.virt.esx.suds.client.Client document store component usage tests."""

    @pytest.mark.parametrize("store", (object(), virtwho.virt.esx.suds.cache.NoCache()))
    def test_reject_invalid_store_class(self, store, monkeypatch):
        ...
        try:
            ...
            assert str(e) == expected_error % (virtwho.virt.esx.suds.store.DocumentStore,)
        finally:
            del e  # explicitly break circular reference chain in Python 3


class TestTransportUsage:
    """virtwho.virt.esx.suds.client.Client transport component usage tests."""

    def test_default_transport(self):
        client = testutils.client_from_wsdl(testutils.wsdl(""))
        expected = virtwho.virt.esx.suds.transport.https.HttpAuthenticated
        assert client.options.transport.__class__ is expected

    @pytest.mark.parametrize("exception", (
        MyException(),  # non-TransportError exception
        virtwho.virt.esx.suds.transport.TransportError("huku", 666)))
    def test_error_on_open(self, monkeypatch, exception):
        ...
        e_info = pytest.raises(exception.__class__, virtwho.virt.esx.suds.client.Client, "url",
    ...
    # detailed. virtwho.virt.esx.suds.transport.TransportError exceptions get handled in a much
    ...
    #    virtwho.virt.esx.suds.options.faults suds option
    ...
    # just demonstrates that virtwho.virt.esx.suds.transport.TransportError exceptions get
    # handled differently from other exception types.
    def test_error_on_send__transport(self, monkeypatch):
        monkeypatch.delitem(locals(), "e", False)
        t = MockTransport(send_data=virtwho.virt.esx.suds.transport.TransportError("huku", 666))
        store = MockDocumentStore(wsdl=testutils.wsdl("", operation_name="g"))
        client = virtwho.virt.esx.suds.client.Client("suds://wsdl", documentStore=store,
            cache=None, transport=t)
  ...
  <env:Body>
    ...
    def test_reject_invalid_transport_class(self, transport, monkeypatch):
        ...
        try:
            ...
            assert str(e) == expected_error % (virtwho.virt.esx.suds.transport.Transport,)
        finally:
            del e  # explicitly break circular reference chain in Python 3
    ...
    def test_WSDL_transport(self, url):
        ...
        virtwho.virt.esx.suds.client.Client(url, cache=None, documentStore=store, transport=t)
...
class TestWSDLImportWithDifferentTargetNamespaces:
    ...
    def test_imported_entity_reference_using_base_namespace(self):
        ...
        store = MockDocumentStore(wsdl=wsdl_import_wrapper,
            wsdl_imported=wsdl_imported)
        e = pytest.raises(Exception, virtwho.virt.esx.suds.client.Client, "suds://wsdl",
...
<wsdl:definitions targetNamespace="%(tns)s"
  ...
  <wsdl:service name="my-service">
    ...
    def test_recursive_WSDL_import_with_single_URL_per_WSDL(self):
        ...
        virtwho.virt.esx.suds.client.Client(url_main, cache=None, documentStore=store)
    ...
    def test_recursive_WSDL_import_with_multiple_URLs_per_WSDL(self):
        ...
        store = MockDocumentStore(wsdl_main_1=wsdl_main, wsdl_main_2=wsdl_main,
            wsdl_binding=wsdl_binding)
        virtwho.virt.esx.suds.client.Client(url_main1, cache=None, documentStore=store)
