...
def test_ACCEPTED_and_NO_CONTENT_status_reported_as_None_with_faults():
    ...
    def f(reply, status):
        inject = {"reply": virtwho.virt.esx.suds.byte_str(reply), "status": status}
...
def test_badly_formed_reply_XML():
    for faults in (True, False):
        ...
        pytest.raises(xml.sax.SAXParseException, client.service.f,
            __inject={"reply": virtwho.virt.esx.suds.byte_str("bad food")})
...
def test_restriction_data_types():
    ...
    for client in (client_unnamed, client_named, client_twice_restricted):
        response = client.service.f(__inject=dict(reply=virtwho.virt.esx.suds.byte_str("""\
<?xml version="1.0"?>
...
<Envelope xmlns="http://schemas.xmlsoap.org/soap/envelope/">
  <Body>
    ...
    assert response.__class__.__bases__[0] is virtwho.virt.esx.suds.sudsobject.Object
    assert response.Elemento.__class__ is virtwho.virt.esx.suds.sax.text.Text
...
def test_empty_reply():
    ...
    def f(status=None, description=None):
        inject = dict(reply=virtwho.virt.esx.suds.byte_str(), status=status,
...
""") % (unicode_string,))
    ...
    e = pytest.raises(virtwho.virt.esx.suds.WebFault, client.service.f, __inject=inject).value
    try:
        assert e.fault.faultstring == unicode_string
        assert e.document.__class__ is virtwho.virt.esx.suds.sax.document.Document
    finally:
...
def test_missing_wrapper_response():
    ...
    response_with_missing_wrapper = client.service.f(__inject=dict(
        reply=virtwho.virt.esx.suds.byte_str("""<?xml version="1.0"?>
<Envelope xmlns="http://schemas.xmlsoap.org/soap/envelope/">
  <Body>
...
def test_reply_error_with_detail_with_fault(monkeypatch):
    ...
    for http_status in (http_client.OK, http_client.INTERNAL_SERVER_ERROR):
        ...
        e = pytest.raises(virtwho.virt.esx.suds.WebFault, client.service.f, __inject=inject)
        try:
            e = e.value
...
<Envelope xmlns="http://schemas.xmlsoap.org/soap/envelope/">
  <Body>
    ...
    assert response_bare.__class__ is virtwho.virt.esx.suds.sax.text.Text
...
<Envelope xmlns="http://schemas.xmlsoap.org/soap/envelope/">
  <Body>
    ...
    assert response_wrapped.__class__ is virtwho.virt.esx.suds.sax.text.Text
...
<Envelope xmlns="http://schemas.xmlsoap.org/soap/envelope/">
  <Body>
    ...
    assert response.result1.__class__ is virtwho.virt.esx.suds.sax.text.Text
    assert response.result2.__class__ is virtwho.virt.esx.suds.sax.text.Text
    assert response.result3.__class__ is virtwho.virt.esx.suds.sax.text.Text
...
_fault_reply__with_detail = virtwho.virt.esx.suds.byte_str("""\
<?xml version="1.0"?>
<env:Envelope xmlns:env="http://schemas.xmlsoap.org/soap/envelope/">
  <env:Body>
    <env:Fault>
...
_fault_reply__without_detail = virtwho.virt.esx.suds.byte_str("""\
<?xml version="1.0"?>
<env:Envelope xmlns:env="http://schemas.xmlsoap.org/soap/envelope/">
  <env:Body>
    <env:Fault>
