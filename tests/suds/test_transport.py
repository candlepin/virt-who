...
from virtwho.virt.esx.suds.transport import Reply, Request, Transport
import virtwho.virt.esx.suds.transport.options
...
class TestBaseTransportClass:
    ...
    def test_members(self):
        t = Transport()
        assert t.options.__class__ is virtwho.virt.esx.suds.transport.options.Options
...
%s""") % (url, request.headers, message.decode("raw_unicode_escape"))
    ...
    def test_string_representation_with_no_message(self):
        ...
        headers = {virtwho.virt.esx.suds.byte_str("yuck"): virtwho.virt.esx.suds.byte_str("ptooiii...")}
        request = Request(url)
        request.headers = headers
        expected = u("""\
URL: %s
