...
class SendMethodFixture:
    ...
    Each instance is connected to a specific virtwho.virt.esx.suds.transport.http.HttpTransport
    request sending method and may be used to call that method on a specific
    virtwho.virt.esx.suds.transport.http.HttpTransport instance.
    ...
    def __call__(self, transport, *args, **kwargs):
        assert isinstance(transport, virtwho.virt.esx.suds.transport.http.HttpTransport)
...
def send_method(request):
    ...
    fixture to be called for each virtwho.virt.esx.suds.transport.http.HttpTransport request
    sending method.
...
def test_authenticated_http_add_credentials_to_request(input):
    ...
    t = virtwho.virt.esx.suds.transport.http.HttpAuthenticated(**input)
    r = MockRequest()
    t.addcredentials(r)
...
def test_construct_authenticated_http(input):
    ...
    transport = virtwho.virt.esx.suds.transport.http.HttpAuthenticated(**input)
...
def test_request_headers_are_passed_to_urllib_when_opening_a_request():
    ...
    transport = virtwho.virt.esx.suds.transport.http.HttpTransport()
    transport.urlopener = MockURLOpenerSaboteur(MyException)

    pytest.raises(MyException, virtwho.virt.esx.suds.transport.http.HttpTransport.open, transport, mock_request)
...
def test_sending_using_network_sockets(send_method, monkeypatch):
    ...
    class Mocker(CountedMock):
        ...
        def mock_reset(self):
            ...
            self.mock_sent_data = virtwho.virt.esx.suds.byte_str()
    ...
    request = virtwho.virt.esx.suds.transport.Request(url, non_ascii_byte_data)
    transport = virtwho.virt.esx.suds.transport.http.HttpTransport()
    ...
    assert mocker.mock_sent_data.__class__ is virtwho.virt.esx.suds.byte_str_class
    expected_sent_data_start = "%s /%s HTTP/1.1\r\n" % (
        expected_sent_data_start, url_relative)
    expected_sent_data_start = virtwho.virt.esx.suds.byte_str(expected_sent_data_start)
...
class TestURLOpenerUsage:
    """
    Test demonstrating how virtwho.virt.esx.suds.transport.http.HttpTransport makes use of the
    ...
    def test_open_propagating_HTTPError_exceptions(self, status_code,
        ...
        exceptions to virtwho.virt.esx.suds.transport.TransportError exceptions.
        ...
        t = virtwho.virt.esx.suds.transport.http.HttpTransport()
        t.urlopener = MockURLOpenerSaboteur(open_exception=e_original)
        request = create_request()

        # Execute.
        e = pytest.raises(virtwho.virt.esx.suds.transport.TransportError, t.open, request).value
        try:
            # Verify.
    ...
    def test_operation_invoke_with_urlopen_accept_no_content__data(self,
            status_code):
        """
        virtwho.virt.esx.suds.client.Client web service operation invocation expecting output
        ...
        pytest.raises(virtwho.virt.esx.suds.transport.TransportError, client.service.f)
    ...
    def test_operation_invoke_with_urlopen_accept_no_content__no_data(self,
            status_code):
        """
        virtwho.virt.esx.suds.client.Client web service operation invocation expecting no output
        data, and for which a corresponding urlopen call raises a HTTPError
    ...
    def test_send_transforming_HTTPError_exceptions(self, status_code,
        ...
        virtwho.virt.esx.suds.transport.TransportError exceptions.
        ...
        # Execute.
        e = pytest.raises(virtwho.virt.esx.suds.transport.TransportError, t.send, request).value
        try:
            # Verify.
            assert len(e.args) == 1
...
def _encode_basic_credentials(username, password):
    ...
    """
    data = virtwho.virt.esx.suds.byte_str("%s:%s" % (username, password))
