...
def client_from_wsdl(wsdl_content, *args, **kwargs):
    ...
    """
    assert wsdl_content.__class__ is virtwho.virt.esx.suds.byte_str_class, "bad test data"
    ...
    return virtwho.virt.esx.suds.client.Client("suds://" + test_file_id, *args, **kwargs)
...
if virtwho.virt.esx.suds.__version__ != %(virtwho.virt.esx.suds.__version__)r:
    print("Unexpected suds version imported - '%%s'." %% (virtwho.virt.esx.suds.__version__))
    sys.exit(-2)
...
if sys.version_info >= (3, 0):
    ...
    exec_file = execfile
exec_file(%(script)r)
""" % {"virtwho.virt.esx.suds.__version__": virtwho.virt.esx.suds.__version__,
    "script": script.basename,
    "sys.path": sys_path})
    # the err exception is because we want to run not as root
    if popen.returncode != 0 or (err and not err.startswith("Unable to log to")) or out:
...
""" % (web_service_URL,))
    ...
    return virtwho.virt.esx.suds.byte_str("\n".join(wsdl))
