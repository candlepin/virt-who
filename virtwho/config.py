...
VW_TYPES = ("libvirt", "vdsm", "esx", "rhevm", "hyperv", "fake", "xen", "kubevirt", "nutanix")
...
class VirtConfigSection(ConfigSection):
    ...
    def _validate_server(self, key):
        ...
        """
        result = None
        # Server option must be there for ESX, RHEVM, HYPERV and NUTANIX
    ...
    def _validate_env(self, key):
        ...
        if sm_type == 'sam' and (
                (virt_type in ('esx', 'rhevm', 'hyperv', 'xen', 'nutanix')) or
    ...
    def _validate_owner(self, key):
        ...
        sm_type = self._values['sm_type']
        virt_type = self._values.get('type')
        if sm_type == 'sam' and virt_type in ('esx', 'rhevm', 'hyperv', 'xen', 'nutanix'):
            if key not in self:
                result = (
                    'warning',
                    "Option `%s` needs to be set in config: '%s'" % (key, self.name)
