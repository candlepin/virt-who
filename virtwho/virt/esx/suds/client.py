...
import virtwho.virt.esx.suds.bindings.binding
from virtwho.virt.esx.suds.builder import Builder
import virtwho.virt.esx.suds.cache
import virtwho.virt.esx.suds.metrics as metrics
from virtwho.virt.esx.suds.options import Options
from virtwho.virt.esx.suds.plugin import PluginContainer
from virtwho.virt.esx.suds.properties import Unskin
from virtwho.virt.esx.suds.reader import DefinitionsReader
from virtwho.virt.esx.suds.resolver import PathResolver
from virtwho.virt.esx.suds.sax.document import Document
import virtwho.virt.esx.suds.sax.parser
from virtwho.virt.esx.suds.servicedefinition import ServiceDefinition
import virtwho.virt.esx.suds.transport
import virtwho.virt.esx.suds.transport.https
from virtwho.virt.esx.suds.umx.basic import Basic as UmxBasic
from virtwho.virt.esx.suds.wsdl import Definitions
...
class Client(UnicodeMixin):
    ...
    def __init__(self, url, **kwargs):
        ...
        """
        options = Options()
        options.transport = virtwho.virt.esx.suds.transport.https.HttpAuthenticated()
        self.options = options
        if "cache" not in kwargs:
            kwargs["cache"] = virtwho.virt.esx.suds.cache.ObjectCache(days=1)
    ...
    def __unicode__(self):
        ...
        s.append("  version: %s" % (virtwho.virt.esx.suds.__version__,))
        if virtwho.virt.esx.suds.__build__:
            s.append("  build: %s" % (virtwho.virt.esx.suds.__build__,))
        for sd in self.sd:
            s.append("\n\n%s" % (str(sd),))
        return "".join(s)
...
class _SoapClient:
    ...
    def send(self, soapenv, timeout=None):
        ...
        request = virtwho.virt.esx.suds.transport.Request(location, soapenv, timeout)
        ...
        except virtwho.virt.esx.suds.transport.TransportError as e:
    ...
    def __get_fault(self, replyroot):
        ...
        """
        envns = virtwho.virt.esx.suds.bindings.binding.envns
...
class _SimClient(_SoapClient):
    ...
    def invoke(self, args, kwargs):
        ...
        if msg is not None:
            assert msg.__class__ is virtwho.virt.esx.suds.byte_str_class
        ...
        if reply is not None:
            assert reply.__class__ is virtwho.virt.esx.suds.byte_str_class
...
def _parse(string):
    ...
    """
    if string:
        return virtwho.virt.esx.suds.sax.parser.Parser().parse(string=string)
